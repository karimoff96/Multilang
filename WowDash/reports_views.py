...
def financial_reports(request):
    ...
    completed_revenue = float(completed_orders.aggregate(total=Sum('total_price'))['total'] or 0)
    ...
    if hasattr(request, 'admin_profile') and request.admin_profile:
        if request.admin_profile.is_owner or request.user.is_superuser:
            branch_data = orders.values('branch__name').annotate(
                revenue=Sum('total_price'),
                count=Count('id')
            ).order_by('-revenue')[:10]
...
def order_reports(request):
    ...
    language_breakdown = orders.values('language__name').annotate(
        count=Count('id')
    ).order_by('-count')[:5]
    ...
    for item in language_breakdown:
        language_data.append({
            'pair': item['language__name'] or 'N/A',
            'count': item['count']
        })
    
    # Recent orders list
    recent_orders = orders.select_related('bot_user', 'product', 'assigned_to__user').order_by('-created_at')[:20]
...
def staff_performance(request):
    ...
    if request.user.is_superuser:
        ...
        if request.admin_profile.is_owner:
            staff_members = AdminUser.objects.filter(
                branch__center__owner=request.user,
                is_active=True
            )
        elif request.admin_profile.is_manager:
            staff_members = AdminUser.objects.filter(
                branch=request.admin_profile.branch,
                is_active=True
            )
        else:
    ...
    for staff in staff_members:
        ...
        revenue = float(staff_orders.filter(status='completed').aggregate(total=Sum('total_price'))['total'] or 0)
        
        # Calculate average completion time (simplified - based on updated_at - created_at)
        completion_rate = round((completed_orders / total_assigned * 100) if total_assigned > 0 else 0, 1)
...
def customer_analytics(request):
    ...
    acquisition_data = customers.filter(
        created_at__date__gte=date_from,
        created_at__date__lte=date_to
    ).annotate(
        date=TruncDate('created_at')
    ...
    top_customers = orders.values(
        'bot_user__id', 'bot_user__name', 'bot_user__is_agency'
    ).annotate(
...
def export_report(request, report_type):
    ...
    if report_type == 'orders':
        ...
        response['Content-Disposition'] = f'attachment; filename="orders_report_{date_from}_to_{date_to}.csv"'
        
        writer = csv.writer(response)
        writer.writerow(['Order ID', 'Customer', 'Product', 'Status', 'Total Price', 'Created At'])
        ...
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="financial_report_{date_from}_to_{date_to}.csv"'

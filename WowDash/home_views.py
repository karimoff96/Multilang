...
def index(request):
    ...
    yesterday_orders = all_orders.filter(created_at__gte=yesterday_start, created_at__lt=today_start)
    ...
    yesterday_revenue = float(yesterday_orders.aggregate(total=Sum('total_price'))['total'] or 0)
    
    monthly_count = monthly_orders.count()
    monthly_revenue = float(monthly_orders.aggregate(total=Sum('total_price'))['total'] or 0)
    ...
    recent_orders = all_orders.select_related('bot_user', 'product').order_by('-created_at')[:5]
    recent_orders_data = []
...
def sales(request):
    ...
    today_status_counts = get_status_counts(all_orders.filter(created_at__gte=today_start))
    weekly_status_counts = get_status_counts(all_orders.filter(created_at__gte=week_start))
    monthly_status_counts = get_status_counts(all_orders.filter(created_at__gte=month_start))
    yearly_status_counts = get_status_counts(all_orders.filter(created_at__gte=year_start))
    ...
    def get_top_users(orders_queryset, is_agency=True, limit=5):
        ...
        return orders_queryset.filter(
            bot_user__is_agency=is_agency
        ).values(
            'bot_user__id',
            'bot_user__name',
            'bot_user__username',
    ...
    def serialize_top_users(queryset):
        ...
        for user in queryset:
            result.append({
                ...
                'joined': user['bot_user__created_at'].strftime('%d %b %Y') if user['bot_user__created_at'] else '-',
    ...
    top_agencies_data = {
        "today": serialize_top_users(get_top_users(all_orders.filter(created_at__gte=today_start), is_agency=True)),
        "weekly": serialize_top_users(get_top_users(all_orders.filter(created_at__gte=week_start), is_agency=True)),
        "monthly": serialize_top_users(get_top_users(all_orders.filter(created_at__gte=month_start), is_agency=True)),
        "yearly": serialize_top_users(get_top_users(all_orders.filter(created_at__gte=year_start), is_agency=True)),
    }
    
    top_customers_data = {
        "today": serialize_top_users(get_top_users(all_orders.filter(created_at__gte=today_start), is_agency=False)),
        "weekly": serialize_top_users(get_top_users(all_orders.filter(created_at__gte=week_start), is_agency=False)),
        "monthly": serialize_top_users(get_top_users(all_orders.filter(created_at__gte=month_start), is_agency=False)),
        "yearly": serialize_top_users(get_top_users(all_orders.filter(created_at__gte=year_start), is_agency=False)),
    }
    ...
    yesterday_start = today_start - timedelta(days=1)
    yesterday_orders = all_orders.filter(created_at__gte=yesterday_start, created_at__lt=today_start)
    yesterday_revenue = yesterday_orders.aggregate(total=Sum('total_price'))['total'] or 0
    
    prev_week_start = week_start - timedelta(days=7)
    prev_week_orders = all_orders.filter(created_at__gte=prev_week_start, created_at__lt=week_start)
    prev_week_revenue = prev_week_orders.aggregate(total=Sum('total_price'))['total'] or 0
    
    prev_month_start = (month_start - timedelta(days=1)).replace(day=1)
    prev_month_orders = all_orders.filter(created_at__gte=prev_month_start, created_at__lt=month_start)
    prev_month_revenue = prev_month_orders.aggregate(total=Sum('total_price'))['total'] or 0
    
    prev_year_start = year_start.replace(year=year_start.year - 1)
    prev_year_orders = all_orders.filter(created_at__gte=prev_year_start, created_at__lt=year_start)
    prev_year_revenue = prev_year_orders.aggregate(total=Sum('total_price'))['total'] or 0
    ...
    for i in range(11, -1, -1):
        ...
        m_revenue = all_orders.filter(
            created_at__gte=m_start, 
            created_at__lt=m_end
        ).aggregate(total=Sum('total_price'))['total'] or 0
        yearly_chart_data.append(float(m_revenue))
        yearly_chart_labels.append(m_start.strftime('%b'))
    
    # Monthly chart data (days of current month)
    import calendar
    ...
    for day in range(1, days_in_current_month + 1):
        ...
        if day_start > now:
            break
        d_revenue = all_orders.filter(
            created_at__gte=day_start,
            created_at__lt=day_end
        ).aggregate(total=Sum('total_price'))['total'] or 0
        monthly_chart_data.append(float(d_revenue))
    ...
    for i in range(7):
        ...
        weekly_chart_data.append(float(d_revenue))
        weekly_chart_labels.append(day_names[i])
    
    # Today chart data (24 hours)
    today_chart_data = []
    today_chart_labels = []
    for hour in range(24):
        ...
        if hour_start > now:
            break
        h_revenue = all_orders.filter(
            created_at__gte=hour_start,
            created_at__lt=hour_end
        ).aggregate(total=Sum('total_price'))['total'] or 0
        today_chart_data.append(float(h_revenue))
        today_chart_labels.append(f'{hour:02d}:00')
    
    # ============ RECENT ORDERS ============
    recent_orders = all_orders.select_related('bot_user', 'product').order_by('-created_at')[:10]
    recent_orders_data = []
    for order in recent_orders:
        recent_orders_data.append({
            'id': order.id,
    ...
    def get_completion_rate(orders_queryset):
        ...
        in_progress = orders_queryset.filter(status__in=['pending', 'payment_pending', 'payment_received', 'payment_confirmed', 'in_progress', 'ready']).count()
...
def finance(request):
    ...
    def get_payment_breakdown(orders_queryset):
        ...
        result = {'cash': {'count': 0, 'revenue': 0}, 'card': {'count': 0, 'revenue': 0}}
    ...
    payment_breakdown = {
        ...
        'monthly': get_payment_breakdown(all_orders.filter(created_at__gte=month_start)),
    ...
    def get_user_type_revenue(orders_queryset):
        ...
        return {
            'agency': {'count': agency['count'] or 0, 'revenue': float(agency['revenue'] or 0)},
            'regular': {'count': regular['count'] or 0, 'revenue': float(regular['revenue'] or 0)}
        }
    ...
    user_type_revenue = {
        ...
        'monthly': get_user_type_revenue(all_orders.filter(created_at__gte=month_start)),
        'yearly': get_user_type_revenue(all_orders.filter(created_at__gte=year_start)),
    }
    
    # ============ REVENUE BY PRODUCT ============
    def get_product_revenue(orders_queryset, limit=5):
        products = orders_queryset.values(
            'product__id', 'product__name'
        ).annotate(
            count=Count('id'),
            revenue=Sum('total_price'),
            pages=Sum('total_pages')
    ...
    pending_payments = all_orders.filter(
        payment_type='card',
        status__in=['payment_pending', 'payment_received']
    ).aggregate(
        count=Count('id'),
    ...
    prev_month_start = (month_start - timedelta(days=1)).replace(day=1)
    prev_month_stats = get_revenue_stats(all_orders.filter(
    ...
    if prev_month_stats['revenue'] > 0:
        monthly_revenue_change = round(
            ((monthly_stats['revenue'] - prev_month_stats['revenue']) / prev_month_stats['revenue']) * 100, 1
        )
